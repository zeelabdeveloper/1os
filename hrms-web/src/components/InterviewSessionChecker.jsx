...
const InterviewSessionNotifications = () => {
  ...
  const grouped = useMemo(() => ({
    upcoming: sessions.filter((s) => ["scheduled", "in_progress"].includes(s.status)),
    completed: sessions.filter((s) => s.status === "completed"),
    others: sessions.filter((s) => !["scheduled", "in_progress", "completed"].includes(s.status)),
  }), [sessions]);

  const handleChange = useCallback((id, value, type) => {
    mutation.mutate({ id, data: type === "status" ? { status: value } : { status: value, isOutCome: true } });
  }, [mutation]);
  ...
  const CardHeader = useCallback(({ session, showSelect }) => (
    <div className="flex justify-between items-start">
      <div>
        ...
        <Space size="middle">
          <Avatar src={session.applicationId?.photo} icon={<UserOutlined />} size="small" />
          <Text className="cursor-pointer !text-blue-700" onClick={() => navigate(`/recruitment/application?id=${session.applicationId._id}`)}>
      ...
      <div className="text-right">
        ...
        {session.meetingLink && (
          <Button type="link" href={session.meetingLink} target="_blank" className="p-0">
            Join Meeting
          </Button>
        )}
      </div>
  ...
  const OutcomeSelect = useCallback(({ session }) => (
    <Select
  ...
  const SessionCard = useCallback(({ session, showStatusSelect, outcomeSelect }) => (
    <Card
      key={session._id}
      className={`border ${session.status === "cancelled" ? "border-red-100 bg-red-50" : "border-gray-100"}`}
      actions={outcomeSelect ? [<Button type="text" icon={<EditOutlined />} onClick={() => openEdit(session)}>Add Feedback</Button>] : []}
    >
      <CardHeader session={session} showSelect={showStatusSelect} />
      {outcomeSelect && <div className="mt-2"><OutcomeSelect session={session} /></div>}
      {session.notes && (
        <div className="mt-3">
          <Text type="secondary" className="block text-sm">Notes:</Text>
    ...
    <div className="p-4">
      <Card title="Interview Notifications" bordered={false}>
        <Space direction="vertical" className="w-full">
          {["upcoming", "completed", "others"].map((type) => grouped[type]?.length > 0 && (
            <div key={type}>
              <Text strong className="text-lg capitalize">{type} Interviews</Text>
              <Divider />
